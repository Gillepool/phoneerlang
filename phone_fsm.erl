...
-export([start_link/1, stop/1, connect/1, disconnect/1, phone_action/2,
        ...
        handle_event/3, init/1, handle_sync_event/4,
        idle/2, calling/2, handle_info/3, terminate/3, receiving/2,
        connected/2]).
-behaviour(gen_fsm).
...
start_link(PhoneNumber) ->
    case gen_fsm:start_link(?MODULE, PhoneNumber, []) of
        {ok, Pid} -> {ok, Pid};
        {error, {_, Pid}} -> {ok, Pid}
    end.
...
connect(FsmPid) ->
    gen_fsm:sync_send_all_state_event(FsmPid, connect),
    ok.
disconnect(FsmPid) ->
    gen_fsm:sync_send_all_state_event(FsmPid, disconnect),
    ok.
...
phone_action(FsmPid, Event) ->
    case Event of
        {outbound, PhoneNumber} ->
            gen_fsm:send_event(FsmPid, {event, {outbound, PhoneNumber}});
        reject ->
            gen_fsm:send_event(FsmPid, {event, reject});
        accept ->
            gen_fsm:send_event(FsmPid, {event, accept});
        hangup ->
            io:format('hangup'),
            gen_fsm:send_event(FsmPid, {event, hangup})
    end.


%% sending events between controllers, internal events
inbound(FsmPid) ->
    gen_fsm:send_event(FsmPid, {event, {inbound, self()}}). %% seding inbound message with own pid
accept(FsmPid) ->
    gen_fsm:send_event(FsmPid, {event, accept}).
hangup(FsmPid) ->
    gen_fsm:send_event(FsmPid, {event, hangup}).
reject(FsmPid) ->
    gen_fsm:send_event(FsmPid, {event, record}).
busy(FsmPid) ->
    gen_fsm:send_event(FsmPid, {event, busy}).


%%  stop has been sent
%% stop whatever we're doing and shut down!
handle_event(stop, StateName, S=#phoneState{}) ->
...
%% idle state
idle({event, Event}, #phoneState{phone=Phone} = State) ->
  case Event of
